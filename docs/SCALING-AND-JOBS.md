...
| **RAG / PGVector chat** | **Shipped (Phase 1):** migration `019`, index-on-complete, lazy index on chat if chunks missing, `POST /api/runs/{id}/chat`, results UI. Off until `RAG_ENABLED=true`. **Per-user Ask-docs token meter:** 100k/month (`rag_chat` usage events; query embed + answer LLM; index-on-complete / lazy index not charged to the user pool — only global `OPENAI_DAILY_BUDGET_USD`) | Hybrid search / re-rank deferred. **Local caveat:** if local API shares Upstash Redis with Railway, the **production worker** may finish the job without RAG — chat lazy-indexes from `cached_documents` as a fallback | Product demand | Stay on Supabase PGVector; embeddings under same daily budget. For pure local RAG testing, unset `REDIS_URL` so runs execute in-process |
